...
{
    internal class TicketValidationTravelEvent : IEvent
    {
        private readonly IEnvironment _environment;
        private readonly IConfiguration _configuration;
        private readonly Random _random;
        public string EventName { get; init; } = "TICKET_VALIDATION_TRAVEL_EVENT";
        public string EventDescription { get; init; } = "Passeger travels to tocket validation";
        ...
        public TicketValidationTravelEvent(IEnvironment environment, IConfiguration configuration, Random random, PermanentEntity? server, TemporaryEntity temporaryEntity, ITimeInstant timeInstant)
        {
        ...
        public void EventHandler(ITimeInstant handlingTimeInstant)
        {
            ...
            var minTime = _configuration.GetValue<int>("Simulation:Events:TicketValidation:Travel:NextEventMinimumInterval");
            var maxTime = _configuration.GetValue<int>("Simulation:Events:TicketValidation:Travel:NextEventMaximumInterval");
            var timeToStartNextEvent = HandlingTimeInstant.Time + _random.Next(minTime, maxTime);

            _environment.Events.Add(new TicketValidationStartEvent(_environment, _configuration, _random, null, TemporaryEntity, new TimeInstant(timeToStartNextEvent)));
