...
{
    internal class PlaneEntranceTravelEvent : IEvent
    {
        private readonly IEnvironment _environment;
        private readonly IConfiguration _configuration;
        private readonly Random _random;
        public string EventName { get; init; } = "PLANE_ENTRANCE_TRAVEL_EVENT";
        public string EventDescription { get; init; } = "Passeger travels to plane entrance";
        ...
        public PlaneEntranceTravelEvent(IEnvironment environment, IConfiguration configuration, Random random, PermanentEntity? server, TemporaryEntity temporaryEntity, ITimeInstant timeInstant)
        {
        ...
        public void EventHandler(ITimeInstant handlingTimeInstant)
        {
            ...
            var minTime = _configuration.GetValue<int>("Simulation:Events:PlaneEntrance:Travel:NextEventMinimumInterval");
            var maxTime = _configuration.GetValue<int>("Simulation:Events:PlaneEntrance:Travel:NextEventMaximumInterval");
            var timeToStartNextEvent = HandlingTimeInstant.Time + _random.Next(minTime, maxTime);

            _environment.Events.Add(new PlaneEntranceStartEvent(_environment, _configuration, _random, null, TemporaryEntity, new TimeInstant(timeToStartNextEvent)));
