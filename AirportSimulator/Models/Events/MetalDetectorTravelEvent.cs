...
{
    internal class MetalDetectorTravelEvent : IEvent
    {
        private readonly IEnvironment _environment;
        private readonly IConfiguration _configuration;
        private readonly Random _random;
        public string EventName { get; init; } = "METAL_DETECTOR_TRAVEL_EVENT";
        public string EventDescription { get; init; } = "Passeger travels to metal detector";
        ...
        public MetalDetectorTravelEvent(IEnvironment environment, IConfiguration configuration, Random random, PermanentEntity? server, TemporaryEntity temporaryEntity, ITimeInstant timeInstant)
        {
        ...
        public void EventHandler(ITimeInstant handlingTimeInstant)
        {
            ...
            var minTime = _configuration.GetValue<int>("Simulation:Events:MetalDetector:Travel:NextEventMinimumInterval");
            var maxTime = _configuration.GetValue<int>("Simulation:Events:MetalDetector:Travel:NextEventMaximumInterval");
            var timeToStartNextEvent = HandlingTimeInstant.Time + _random.Next(minTime, maxTime);

            _environment.Events.Add(new MetalDetectorStartEvent(_environment, _configuration, _random, null, TemporaryEntity, new TimeInstant(timeToStartNextEvent)));
